...
def fit(train_data, config):
    ...
    hour = 2 * (train_data["time.hour"].values / 23.0) - 1
    hour = torch.tensor(hour, dtype=torch.float32).reshape(-1, 1, 1).repeat(1, len(train_data.station), 1)
    ...
    cnp = DeepTime(3, 1, config["hidden_dims"], config["repr_dim"], config["dropout"]).to(config["device"])
...
def predict(test_data, train_data, config):
    ...
    def prepare(data):
        ...
        # add hour of day as a feature
        hour = 2 * (data["time.hour"].values / 23.0) - 1
        hour = torch.tensor(hour, dtype=torch.float32).reshape(-1, 1, 1).repeat(1, len(data.station), 1)
