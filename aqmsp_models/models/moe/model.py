...
class MOE(nn.Module):
    def __init__(self, xdim, n_experts):
        super().__init__()
        self.linear = nn.Linear(xdim, n_experts)
        self.gate = nn.Linear(xdim, n_experts)

    def forward(self, x):
        exp_out = self.linear(x)
        w = F.softmax(self.gate(x), dim=1)
        return (w * exp_out).sum(dim=1, keepdim=True)
...
def fit_predict(train_data, test_data, config):
    def train_fn(ts):
        ...
        model = MOE(xdim=len(config.features), n_experts=config.n_experts)
